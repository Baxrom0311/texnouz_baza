...
def sync_table(local_cursor, remote_cursor, table_name, columns, has_last_id):
    ...
    # Remote DB dan eng oxirgi qabul qilingan ID (max id) ni aniqlash
    remote_cursor.execute(f"SELECT COALESCE(MAX(id), 0) FROM {table_name}")
    last_id = remote_cursor.fetchone()[0]
    ...
    while True:
        # Remote databasedan eng so'nggi ID ni olamiz
        remote_cursor.execute(f"SELECT COALESCE(MAX(id), 0) FROM {table_name}")
        next_remote_id = remote_cursor.fetchone()[0] + 1
